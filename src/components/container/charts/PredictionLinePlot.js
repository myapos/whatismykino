...
class PredictionLinePlot extends Component {
  ...
  render () {
    const { kinos, limited, allKinos, waitForPrediction,
      trendData, prediction, maData, lwmaData } = this.props;

    console.log('waitForPrediction:', waitForPrediction, ' prediction:', prediction);
    ...
    if (prediction.length > 0 && !waitForPrediction) {
      ...
      return (<div>
        <div className="labelForChart">Prediction based on Holt-Winters approach</div>
        <LineChart
